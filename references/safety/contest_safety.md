...
调用大模型 API 属于 AI 使用：必须登记到 `run_manifest.ai_usage[]` 并按当届 AI 政策披露。向外部端点发送当前赛题数据在 `live_contest` 下按“外部写入”策略处理——默认 `deny`，仅当届 profile 显式允许或属于已登记并经人工确认的官方端点例外时放行；规则不明时用 `ask`，不得默认允许。
